...
RGraph.Line = function (conf) {
                ...
                for (var i = 0, len = this.coords2.length; i < len; ++i) {
                    ...
                    for (var j = 0; j < this.coords2[i].length; ++j) {
                        if (typeof(this.coords2[i][j]) == 'object' && typeof(this.coords2[i][j][0]) == 'number' && typeof(this.coords2[i][j][1]) == 'number') {
                            ...
                            this.DrawTick(this.coords2[i], this.coords2[i][j][0], this.coords2[i][j][1], co.strokeStyle, false, j == 0 ? 0 : this.coords2[i][j - 1][0], j == 0 ? 0 : this.coords2[i][j - 1][1], tickmarks, j, i);
                        }
                    }
                }
                co.stroke();
            ...
            for (var i = 0, len = this.coords2.length; i < len; i += 1) {
                for (var j = 0, len2 = this.coords2[i].length; j < len2; ++j) {
                    if (typeof(this.coords2[i][j]) == 'object' && typeof(this.coords2[i][j][0]) == 'number' && typeof(this.coords2[i][j][1]) == 'number') {
                        ...
                        co.strokeStyle = prop['chart.colors'][i];
                        this.DrawTick(this.coords2[i], this.coords2[i][j][0], this.coords2[i][j][1], prop['chart.colors'][i], false, j == 0 ? 0 : this.coords2[i][j - 1][0], j == 0 ? 0 : this.coords2[i][j - 1][1], tickmarks, j, i);
                    }
                }
            }
        }
        ...
        if (prop['chart.noyaxis'] == false && numyticks > 0) {
            ...
            if (prop['chart.xaxispos'] == 'center') {
                ...
                if (prop['chart.ymin'] < 0) {
                    co.moveTo((prop['chart.yaxispos'] == 'left' ? this.gutterLeft : ca.width - this.gutterRight), ma.round(y));
                    co.lineTo(lineto, ma.round(y));
                }
            }
    ...
    this.drawLabels = this.DrawLabels = function () {
        ...
        if (prop['chart.labels'] && prop['chart.labels'].length > 0) {
            ...
            var numLabels = prop['chart.labels'].length, offsetx = prop['chart.labels.offsetx'],
    ...
    this.drawLine = this.DrawLine = function (lineData, color, fill, linewidth, tickmarks, index) {
        ...
        for (i = 0, len = lineData.length; i < len; i += 1) {
            ...
            if (lineData[i] == null || (prop['chart.xaxispos'] == 'bottom' && lineData[i] < this.min && !prop['chart.outofbounds']) || (prop['chart.xaxispos'] == 'center' && lineData[i] < (-1 * this.max) && !prop['chart.outofbounds']) || (((lineData[i] < this.min && prop['chart.xaxispos'] !== 'center') || lineData[i] > this.max) && !prop['chart.outofbounds'])) {
                yPos = null;
            }
            co.lineCap = 'round';
            co.lineJoin = 'round';
    ...
    this.drawTick = this.DrawTick = function (lineData, xPos, yPos, color, isShadow, prevX, prevY, tickmarks, index, dataset) {
        ...
        if (tickmarks == 'circle' || tickmarks == 'filledcircle' || tickmarks == 'endcircle') {
            ...
            pa2(co, ['b', 'a', xPos, yPos, prop['chart.ticksize'], 0, 360 / (180 / RG.PI), false, 'c', 'f', prop['chart.tickmarks.dot.fill'] || color, 's', prop['chart.tickmarks.dot.stroke'] || color]);
    ...
    this.getShape = this.getPoint = function (e) {
        var obj = this, RG = RGraph, ca = canvas = e.target, co = context = this.context, prop = this.properties;
        ...
        for (var i = 0; i < obj.coords.length; ++i) {
            ...
            if (mouseX <= (x + prop['chart.tooltips.hotspot.size']) && mouseX >= (x - prop['chart.tooltips.hotspot.size']) && mouseY <= (y + prop['chart.tooltips.hotspot.size']) && mouseY >= (y - prop['chart.tooltips.hotspot.size'])) {
    ...
    this.drawAboveLabels = this.DrawAboveLabels = function () {
        var size = prop['chart.labels.above.size'], font = prop['chart.labels.above.font'] || prop['chart.text.font'],
            units_pre = prop['chart.labels.above.units.pre'], units_post = prop['chart.labels.above.units.post'],
            ...
            offsety = prop['chart.labels.above.offsety'] + size, specific = prop['chart.labels.above.specific'];
        co.beginPath();
    ...
    this.highlight = this.Highlight = function (shape) {
        if (prop['chart.tooltips.highlight']) {
            if (typeof prop['chart.highlight.style'] === 'function') {
                ...
                RG.path2(obj.context, 'b a % % 13 0 6.2830 false f rgba(255,255,255,0.75)', shape.x, shape.y);
                RG.path2(obj.context, 'ga 0.15 b a % % 13 0 6.2830 false f % ga 1', shape.x, shape.y, color);
                RG.path2(obj.context, 'b a % % 7 0 6.2830 false f white', shape.x, shape.y);
                RG.path2(obj.context, 'b a % % 5 0 6.2830 false f %', shape.x, shape.y, color);
            } else {
    ...
    this.getObjectByXY = function (e) {
        ...
        if ((mouseXY[0] > prop['chart.gutter.left'] - 5) && mouseXY[0] < (ca.width - prop['chart.gutter.right'] + 5) && mouseXY[1] > (prop['chart.gutter.top'] - 5) && mouseXY[1] < (ca.height - prop['chart.gutter.bottom'] + 5)) {
            return this;
        }
    };
    ...
    this.parseColors = function () {
        ...
        var properties = ['chart.background.barcolor1', 'chart.background.barcolor2', 'chart.background.grid.color', 'chart.background.color', 'chart.text.color', 'chart.crosshairs.color', 'chart.annotate.color', 'chart.title.color', 'chart.title.yaxis.color', 'chart.key.background', 'chart.axis.color', 'chart.highlight.fill'];
    ...
    this.interactiveKeyHighlight = function (index) {
        ...
        if (coords) {
            ...
            if (prop['chart.curvy']) {
                ...
                for (var i = 0, len = coords.length; i < len; i += 1) {
                    if (i == 0 || RG.is_null(coords[i][1]) || (typeof coords[i - 1][1] != undefined && RG.is_null(coords[i - 1][1]))) {
                        co.moveTo(coords[i][0], coords[i][1]);
                    } else {
                        co.lineTo(coords[i][0], coords[i][1]);
                    }
        ...
        for (var i = 0; i < coords.length; ++i) {
            ...
            if (typeof errorbars === 'number' || typeof errorbars[i] === 'number') {
                ...
                if (positiveLength || negativeLength) {
                    ...
                    pa2(co, 'lj miter lc square b m % % l % % s %', coords[i][0] - halfwidth, coords[i][1] + negativeLength, coords[i][0] + halfwidth, coords[i][1] + negativeLength, color);
                }
                ...
                if (typeof errorbars[i][1] === 'number') {
                    ...
                    pa2(co, 'b m % % l % % l % % m % % l % % s %', coords[i][0], coords[i][1], coords[i][0], coords[i][1] + negativeLength, coords[i][0] - halfwidth, ma.round(coords[i][1] + negativeLength), coords[i][0], ma.round(coords[i][1] + negativeLength), coords[i][0] + halfwidth, ma.round(coords[i][1] + negativeLength), color);
                }
            }
        }
        co.restore();
    ...
    this.unfoldFromCenterTrace = this.unfoldFromCenterTrace2 = function () {
        var obj = this, opt = arguments[0] || {}, frames = opt.frames || 30, frame = 0,
            data = RG.arrayClone(obj.original_data), callback = arguments[1] || function () {
                };
