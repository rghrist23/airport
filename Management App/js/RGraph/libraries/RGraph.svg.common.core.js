...
(function (win, doc, undefined) {
    ...
    RG.SVG.createSVG = function (opt) {
        ...
        var group = RG.SVG.create({svg: svg, type: 'g', attr: {className: 'all-elements', _transform: 'skewY(5)'}});
                ...
                if (prop.yaxisTickmarks) {
                    ...
                    if (prop.xaxis === false) {
                        ...
                        var axis = RG.SVG.create({
                            ...
                            type: 'path',
                            attr: {
                                d: 'M{1} {2} L{3} {4}'.format(startX, obj.height - prop.gutterBottom, endX, obj.height - prop.gutterBottom - 0.001),
        ...
        if (prop.yaxisScale) {
            ...
            for (var i = 0; i < prop.yaxisLabels.length; ++i) {
                var segment = obj.graphHeight / prop.yaxisLabels.length,
                    ...
                    x = prop.gutterLeft - 7 + prop.yaxisLabelsOffsetx, halign = 'right';
    ...
    RG.SVG.drawBackground = function (obj) {
        ...
        if (prop.backgroundImage) {
            ...
            var img = RG.SVG.create({
                ...
                type: 'image',
                attr: attr,
                style: {opacity: typeof prop.backgroundImageOpacity === 'number' ? prop.backgroundImageOpacity : 1}
            });
        }
    ...
    RG.SVG.attribution = function (obj) {
        ...
        var a = RG.SVG.create({
            svg: obj.svg,
            type: 'a',
            attr: {target: '_blank', 'xlink:href': prop.attributionHref || 'http://www.rgraph.net/'}
        });
    ...
    RG.SVG.random = function (opt) {
        var min = opt.min, max = opt.max, dp = opt.dp || opt.decimals || 0, r = ma.random();
        return Number((((max - min) * r) + min).toFixed(dp));
    };
    RG.SVG.arrayRand = RG.SVG.arrayRandom = RG.SVG.random.array = function (opt) {
        var num = opt.num, min = opt.min, max = opt.max, dp = opt.dp || opt.decimals || 0;
    ...
    RG.SVG.commonSetter = function (opt) {
        var obj = opt.object, name = opt.name, value = opt.value;
