...
RGraph.Drawing.YAxis = function (conf) {
    if (typeof conf === 'object' && typeof conf.x === 'number' && typeof conf.id === 'string') {
        var id = conf.id
        var x = conf.x;
    ...
    this.getShape = function (e) {
        ...
        if (mouseX >= this.x - (prop['chart.align'] == 'right' ? 0 : this.getWidth()) && mouseX <= this.x + (prop['chart.align'] == 'right' ? this.getWidth() : 0) && mouseY >= this.gutterTop && mouseY <= (ca.height - this.gutterBottom)) {
            var x = this.x;
    ...
    this.drawYAxis = this.DrawYAxis = function () {
        ...
        var x = this.x, y = this.gutterTop, height = ca.height - this.gutterBottom - this.gutterTop,
            min = +prop['chart.min'] ? +prop['chart.min'] : 0, max = +prop['chart.max'],
