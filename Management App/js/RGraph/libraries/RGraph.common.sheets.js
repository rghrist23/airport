    ...
    RGraph.Sheets = function (key) {
        ...
        this.get = function (str) {
            ...
            if (str.match(/^([a-z]{1,2})([0-9]+)$/i)) {
                var letter = RegExp.$1, number = RegExp.$2, col = this.get(letter, {trim: false});
                return col[number - 1];
            }
            if (str.match(/^([a-z]{1,2})([0-9]+):([a-z]{1,2})([0-9]+)$/i)) {
                var letter1 = RegExp.$1, number1 = RegExp.$2, letter2 = RegExp.$3, number2 = RegExp.$4
                if (letter1 === letter2) {
                    var cells = [], index = this.getIndexOfLetters(letter1), col = this.col(index, null, {trim: false});
                    for (var i = (number1 - 1); i <= (number2 - 1); ++i) {
                        cells.push(col[i]);
                    }
                } else if (number1 === number2) {
                    var cells = [], row = this.row(number1, null, {trim: false}),
                        index1 = this.getIndexOfLetters(letter1), index2 = this.getIndexOfLetters(letter2)
